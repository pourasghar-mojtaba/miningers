...
if(Configure :: read('acl.gui.roles_permissions.ajax') === true)
{
	...
	$links[] = $this->Html->link(__d('acl', 'Management Roles permissions'), '/admin/acl/aros/ajax_managment_role_permissions', array('class' => ($selected == 'admin_managment_role_permissions' || $selected == 'ajax_managment_role_permissions' )? 'selected' : null));
