 ...
 */
class ShibbolethAuthenticatorComponent extends Component
{
    ...
    */
    ...
    protected function log_user($user)
	{
	    if(isset($user) && is_array($user))
		{
		    ...
		    {
		        if(isset($user[$this->user_model_name]))
		        {
		            $user = $user[$this->user_model_name];
		        }
    ...
    public function get_existing_user()
    {
        if($this->has_shibboleth_session())
        {
            ...
            if(isset($shibboleth_uid) && strlen($shibboleth_uid) > 0)
            {
                ...
                $this->controller->{$this->user_model_name}->recursive = -1;
