 ...
 */
class AlaxosFilterComponent extends Component
{
    ...
    */
	...
	function get_filter()
	{
        ...
        $filter = array();
        if(isset($this->controller->request->data) && count($this->controller->request->data) > 0)
        {
            //Loop for models
            foreach($this->controller->request->data as $model_name => $field_values)
            {
				...
				{
				    /*
				     * Do this test mainly to support the FormHelper->postLink() method that create hidden fields
				     */
				    if(is_array($value))
				    {
				         continue;
				    ...
				    /*
				     * The datetime fields work with an hidden field that contain the value to use
				     * The visible fields can be ignored as they are only useful to build the datetime value
				     */
				    if($this->startsWith($field_name, self :: DATE_PREFIX) || $this->startsWith($field_name, self :: TIME_PREFIX))
				    {
				        continue;
				    }
    	...
    	if(isset($value))
    	{
	    	...
	    	else
	    	{
	    	    ...
	    	    else
	    	    {
	    	         ...
	    	         */
	    	        unset($this->controller->request->data[$model_name][$field_name]);
	    	    }
	    	}
    ...
    function _prepareFilter()
    {
        if(isset($this->controller->request->data) && count($this->controller->request->data) > 0)
        {
            foreach($this->controller->request->data as $model => $fields)
            {
                foreach($fields as $key => $field)
                {
                    ...
                    {
                        unset($this->controller->request->data[$model][$key]);
                    }
                }
            }

            $this->controller->Session->write($this->controller->name . '.' . $this->controller->params['action'], $this->controller->request->data);
        }
        ...
        else
        {
            ...
            else
            {
            	...
            	$this->controller->request->data = $filter;
            }
        }
    }
