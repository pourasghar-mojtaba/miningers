...
if(isset($menu_items))
{
    if(isset($import_css) && $import_css === true)
    {
         ...
         */
        $this->AlaxosHtml->include_css_menu();
    }
    ...
    foreach($menu_items as $key => $menu_item)
    {
        if(isset($menu_item['title']))
        {
            ...
            if(!isset($selected_item))
            {
                ...
                /*
                 * Support of links given as string or array
                 */
                $link    = is_array($link) ? Router :: url($link) : $link;
                $link    = preg_replace('/^' . str_replace('/', '\/', $this->request->base) . '/', '', $link);
                
                $parsed_link = array_merge(array('prefix' => null), Router :: parse($link));
                $parsed_url  = array_merge(array('prefix' => null), Router :: parse($this->request->url));
