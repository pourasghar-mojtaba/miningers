				...
				<div class="box span12">
					...
					<?php echo $this->Form->create('User', array('id'=>'SearchFrom','name'=>'SearchFrom')); ?>
						<div class="row-fluid">
							<div class="span6">
								<div id="DataTables_Table_0_length" class="dataTables_length">
									<label>
									 <a id="all_export_link" href="<?php echo  __SITE_URL."admin/siteviews/alldate_export" ?>"><?php echo $this->Html->image('/img/icons/excel.png'); ?></a>
									...
									<select size="1" aria-controls="DataTables_Table_0" onchange="if (this.value) window.location.href=this.value">
										<?php
											...
											if(isset($_REQUEST['filter']) && $_REQUEST['filter']==50)
												echo "<option value='". __SITE_URL."admin/siteviews/index?filter=50 ' selected='selected'>50</option>";
												else echo"<option value='". __SITE_URL."admin/siteviews/index?filter=50 '>50</option>";	
											if(isset($_REQUEST['filter']) && $_REQUEST['filter']==100)
												echo "<option value='". __SITE_URL."admin/siteviews/index?filter=100 ' selected='selected'>100</option>";
												else echo"<option value='". __SITE_URL."admin/siteviews/index?filter=100 '>100</option>";			
												if(isset($_REQUEST['filter']) && $_REQUEST['filter']==150)
												echo "<option value='". __SITE_URL."admin/siteviews/index?filter=150 ' selected='selected'>150</option>";
												else echo"<option value='". __SITE_URL."admin/siteviews/index?filter=150 '>150</option>";
											if(isset($_REQUEST['filter']) && $_REQUEST['filter']==200)
												echo "<option value='". __SITE_URL."admin/siteviews/index?filter=200 ' selected='selected'>200</option>";
												else echo"<option value='". __SITE_URL."admin/siteviews/index?filter=200 '>200</option>";
										?>
									</select> 
									</label>
								</div>
						...
						<table class="table table-bordered table-striped table-condensed">
							  <thead>
								  <tr>
									  <th><?php echo  $this->Paginator->sort('Siteview.id',__('id'));?></th> 
									  <th><?php echo  $this->Paginator->sort('Siteview.count_view', __('count_view') ) ;?></th>
									  <th><?php echo  $this->Paginator->sort('Siteview.created',__('created'));?></th> 
                              ...
                              //pr($siteviews); 
							  	if(!empty($siteviews))
								{
									foreach($siteviews as $userloglogin)
									{
										
								?>
										<tr>	
										    <td class="center"><?php echo $userloglogin['Siteview']['id']; ?></td>		
											<td class="center"><?php echo $userloglogin['Siteview']['count_view']; ?></td>
