...
public function beforeFilter() {
	parent::beforeFilter();
	$this->Auth->allow(array('register','profile','search','ajax_search','get_notofication_list','profile_follow_payee','profile_chaser','get_last_login_users','search_suggest','new_notification','new_message_count','app_industries','app_search','app_profile','app_profile_chaser','app_profile_follow_payee','get_user_info','new_users','last_users_count','app_edit_profile'));
}
...
public function register() {
	...
	$countries = $this->User->query("
                                    ...
                                    where   Country.name_".$this->Session->read('Config.language')." <> ''
			...
			if($this->User->save($this->request->data))
			{
				...
				*/
				...
				if($this->User->Follow->save($this->request->data)){
					...
					$this->request->data['Notification']['notification_id']= $this->User->Follow->getInsertId();
					$this->request->data['Notification']['from_user_id']= $from_user_id;
        		   ...
        		   $options['joins'] = array(
                    ...
                    if($this->Auth->login($this->request->data)){
						$follow_count = $this->User->Follow->find('count', array('conditions' => array('Follow.from_user_id'=>$from_user_id)));
						...
						$this->Session->write('register_key',$user['User']['register_key']);
						$this->Session->write('mobile',$user['User']['mobile']);
...
function profile($id = null)
	{
		...
		$options['fields'] = array(
				...
				'Industry.id',
				'Industry.title_'.$this->Session->read('Config.language').' as title',
				'Country.name_'.$this->Session->read('Config.language').' as country_name '
			   );
		$options['joins'] = array(
				array('table' => 'industries',
					'alias' => 'Industry',
        ...
        if(($this->request->is('post') || $this->request->is('put')))
    		{
		        ...
		        try{
		            ...
		            $datasource->commit();
		            $User_Info= $this->Session->read('User_Info');
					$this->User->Appuser->recursive = -1;
					$info = $this->User->Appuser->find('first', array('conditions' => array('Appuser.user_id' => $User_Info['id'])));
					if($User_Info['user_type']==1){
						if(empty($info)){
							$this->redirect(__SITE_URL.'appusers/active_app');
						}else if(empty($info['Appuser']['sale_reference_id'])){
							$this->redirect(__SITE_URL.'appusers/active_app');
						}
					}
...
function app_edit_profile($user_id,$lang)
	{
		$response['success'] = false;
		$response['message'] = null;
        ...
        if(($this->request->is('post') || $this->request->is('put')))
    		{
				...
				$this->request->data['User']['user_type'] = $_REQUEST['user_type'];
				$this->request->data['User']['name'] = $_REQUEST['name'];
				$this->request->data['User']['headline'] = $_REQUEST['headline'];
				$this->request->data['User']['industry_id'] = $_REQUEST['industry_id'];
				$this->request->data['User']['site'] = $_REQUEST['site'];
				$this->request->data['User']['details'] = $_REQUEST['details'];
				$this->request->data['User']['country_id'] = $_REQUEST['country_id'];
				$this->request->data['User']['location'] = $_REQUEST['location'];
				$this->request->data['User']['address'] = $_REQUEST['address'];
				$this->request->data['User']['telephon'] = $_REQUEST['telephon'];
				$this->request->data['User']['fax'] = $_REQUEST['fax'];
		        ...
		        try{
		            ...
		            if(!$this->User->exists())
						...
						if(isset($_POST['usertag_id'])){
							$usertag_id=$_POST['usertag_id'];
							$usertag_id=array_filter($usertag_id,'strlen');

							foreach($usertag_id as $id)
							{
						...
						if(!empty($usertag_id) || !empty($tag_id))
						{
							$this->User->Userrelatetag->create();
							if(!$this->User->Userrelatetag->saveMany($data))
		                            throw new Exception(__('the_user_tag_not_saved'),6);
		            ...
		            $response['message'] = 'edit_profile_successfull';
					$response['success'] = TRUE;
					...
					if(in_array($e->getCode(),array(1,2,3,4,5,6,7))){
						...
						$response['message'] = 'edit_profile_notsuccessfull';
		...
		$industries = $this->User->Industry->query("
									SELECT 
											...
											Industry.title_".$lang." as title
									FROM industries AS Industry      
									ORDER BY Industry.title_".$lang." asc");


		$countries = $this->User->query("
									SELECT 
											Country.id, 
											Country.name_".$lang." as name 
											
									FROM countries AS Country    
                                    where   Country.name_".$lang." <> ''
									ORDER BY Country.name_".$lang." asc");
		...
		'countries' => $countries,
		'tags' => $tags,
		'_serialize' => array('response','user','industries','countries','tags')
		));
	}
...
*/
	...
	function logout()
	{
		...
		$this->Session->delete('tag_count');
		$this->Session->delete('mobile');
...
function edit_cover_image(){
	...
	if($file['size']>0){
    	...
    	if(!$output['error'])
    	{
    		...
    		//$this->_edit_cover_crop($cover_image,$_POST['w'],$_POST['h'],$_POST['x'],$_POST['y']);
    	}
        else
        {
            $cover_image='';
...
function _edit_cover_crop($new_image,$w,$h,$x,$y){
	$jpeg_quality = 100;

	$src = __USER_IMAGE_PATH.$new_image;
	...
	if(!imagecopyresampled($dst_r,$img_r,0,0,$x,$y,
		$targ_w,$targ_h,$w,$h))
	{
...
function delete_cover_image(){
	...
	$ret= $this->User->updateAll(
	    array( 'User.cover_image' => '"'.$cover_image.'"',
               'User.cover_x' => '0',
               'User.cover_y' => '0',
               'User.cover_zoom' => '0'  ),   //fields to update
	    array( 'User.id' => $User_Info['id'] )  //condition
	  );
	...
	if($ret)
	{
		...
		echo"<script>
            $('.cropit-image-preview').css('background-image', 'url(".__SITE_URL.'img/'.$cover_image.")'); 
            $('.cropit-image-preview').css('background-position', '0px 0px'); 
            $('.cropit-image-preview').css('background-size', '1400px 350px'); 
            $('.cropit-image-zoom-input').attr('min',0);
            $('.image-editor').cropit({ offset: { x:0,y:0 },zoom:0 });
            $('.image-editor').cropit('reset');
...
function edit_image(){
	...
	if(!$output['error'])
	{
		...
		$this->_crop_profile_image($image,$_POST['w'],$_POST['h'],$_POST['x'],$_POST['y']);
	}
	else
        {
            @unlink(WWW_ROOT."/".__USER_IMAGE_PATH.$image);
 ...
 function _crop_profile_image($new_image,$w,$h,$x,$y){
	$jpeg_quality = 100;
	$src = __USER_IMAGE_PATH.$new_image;
	...
	if($width > 160){
		try{
			$this->Httpupload->resize($new_image,__USER_IMAGE_PATH.$new_image, 160,160,__USER_IMAGE_PATH.__UPLOAD_THUMB.'/'.$new_image, $r, $g, $b);
		} catch (Exception $e) {
...
function get_notofication_list()
{
	 ...
	 $notifications = $this->User->query("
	 	select 
			...
			inner join users as User
					on User.id = Notification.from_user_id
		where Notification.to_user_id  = ".$User_Info['id']."
...
function get_last_login_users(){
	...
	$users = $this->User->query("
				SELECT 
						...
						User.country_id
				FROM users AS User 
			WHERE ( SELECT DATEDIFF(now(),User.last_login) )> 30
	...
	if(!empty($users)){
		foreach($users as $user)
		{
			...
			$last_users=$this->last_users($user['User']['id']);
			$last_users_count=$this->last_users_count($user['User']['id']);
			...
			try{
				...
				$Email->to($user['User']['email']);
				$Email->from(array(__Madaner_Email => __Email_Name));
				$Email->viewVars(array('name'=>$user['User']['name'],'email'=>$user['User']['email'],'user_name'=>$user['User']['user_name'],'image'=>$user['User']['image'],'country_id'=>$user['User']['country_id'],'hot_post_tags'=>$hot_post_tags,'last_users'=>$last_users,'new_message_count'=>$new_message_count,'sex'=>$user['User']['sex'],'notification_count'=>$new_notification,'week_fllower_count'=>$week_fllower_count,'last_users_count'=>$last_users_count));
				$Email->send();
...
public function last_users($user_id){
	 $today = date('Y-m-d h:i:s');

	 $old_week = strtotime($today);
     $old_week = strtotime("-7 day",$old_week);
     $old_week= date('Y-m-d h:i:s',$old_week);
	...
	where User.id <> ".$user_id."
	  and User.created between 	'".$old_week."'  and  '".$today."'
	  limit 0 , 4       
			    ");
	return $users;
}

public function last_users_count($user_id){
	 $today = date('Y-m-d h:i:s');
...
public function hot_post_tags(){
	...
	$today = date('Y-m-d h:i:s');
	$old_week=$this->GilaceDate->subday($today,7);
...
function add_usertag()
 ...
 function get_user_info($user_id = 0)
 {
    ...
    return array('user_info'=>$user,'post_count'=>$post_count,'tofollow_count'=>$tofollow_count,'follow_count'=>$follow_count,'new_message_count'=>$new_message_count,'favorite_count'=>$favorite_count,'tags'=>$tags);
...
function verify_email($email)
 {
	...
	$response['success'] = FALSE;
	$response['message'] = FALSE;
    $email = Sanitize::clean(trim($email));
	/*
	$this->User->recursive = -1;
	$options['fields'] = array(
		'User.id',
	...
	try{
		$Email = new CakeEmail();
		$Email->template('verify_sendemail', 'sendemail_layout');
		$Email->subject(__('welcome_to_madaner'));
		$Email->emailFormat('html');
		$Email->to($register_email);
		$Email->from(array(__Madaner_Email => __Email_Name));
    ...
    try{
       $ret= $this->User->updateAll(
	    array( 'User.email' => '"'.$email.'"','User.register_key' => '-1' ),   //fields to update
	    array( 'User.id' => $User_Info['id'] )  //condition
	   );
	   if($ret){
           $response['success'] = TRUE;
		   $this->Session->write('register_key',-1);
       }
    } catch (Exception $e) {
        $response['success'] = FALSE;
    }

    $response['message'] = $response['success'] ? __('the_email_verify_sucsses') :  __('the_email_verify_notsucsses');
