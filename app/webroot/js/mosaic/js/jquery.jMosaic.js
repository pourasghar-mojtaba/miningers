...
;(function($, window, document, undefined){
	...
	jMosaic.prototype.reverseItems = function() {
		var it = this;
		var link = $(it.element).find("a");
		var img = $(it.element).find(it.options.items_type);
		//var arr = $.makeArray(img);
		 
		//arr.reverse();
		$(it.element).html(link);
		$(it.element).append(img);
	};
	
	jMosaic.prototype.reverseItems1 = function() {
		var it = this;
		var links = $(it.element).find("a");
		var images = $(it.element).find(it.options.items_type);
		
		//$(it.element).html(img);
		//$(it.element).html(links);
		$(it.element).html("");
		for (var i = 0, len = links.length; i < len; i++) {
		    var link = $(links[i]); 
			var img = $(images[i]);
			$(it.element).append(link.append(img)); 
			//console.log(links[i]); 
		}
